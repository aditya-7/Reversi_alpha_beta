...
module.exports = function getAllValidMoves(board, players, opponents, color){
  var X, O;
  if(color == 'W') {
    X = 'X';
    O = 'O';
  ...
  function traverseRecursively(current_board, coin, row_col){
    ...
    if(
      ...
      if(current_board[new_row][new_col] === O){ //check if the current movement gives you a opponent coin
        ...
        if((new_row + row < 8) && (new_col + col < 8) && (new_row + row >= 0) && (new_col + col >= 0)) {
          //if the opponent coin is not at the end of the board after all this, then we can be sure that it is a valid move.
          current_board[new_row][new_col] = X;
          adjacent_coin = {row: new_row, col: new_col};
          allValidMoves.push({board:current_board, move: row_col, coin: adjacent_coin});
        }else{
          //if the opponent coin is at the end of the board, its still not a valid move.
          return false;
        }
      }
    }
  ...
  function addMatrix(valid_move){
    // this method will replace other opponent to player wherever it is needed
  ...
  for (var i = 1; i <= allValidMoves.length; i++) {
    if(final_board_values[allValidMoves[i-1].coin.row + ',' + allValidMoves[i-1].coin.col] !== undefined){
      // if the move has already been played with another player coin
      addMatrix(allValidMoves[i-1]);
    }else{
      // if the move has not been played with another player coin
